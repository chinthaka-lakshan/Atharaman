...
const HotelOwnerForm = ({ owner, onSave, onCancel, isEditing = false, submitting = false }) => {
  const [formData, setFormData] = useState({
    hotel_owner_name: owner?.hotel_owner_name || '',
    hotel_owner_nic: owner?.hotel_owner_nic || '',
    hotel_owner_dob: owner?.hotel_owner_dob || '',
    hotel_owner_address: owner?.hotel_owner_address || '',
  ...
  const validateNic = useCallback(async (nicValue) => {
    ...
    if (isEditing && owner && nicValue === owner.hotel_owner_nic) {
  ...
  useEffect(() => {
    const nicValue = formData.hotel_owner_nic;
  ...
  }, [formData.hotel_owner_nic, validateNic]);
  ...
  const handleSubmit = async (e) => {
    ...
    // Append all hotel owner fields
    formDataObj.append('hotel_owner_name', formData.hotel_owner_name);
    formDataObj.append('hotel_owner_nic', formData.hotel_owner_nic);
    formDataObj.append('hotel_owner_dob', formData.hotel_owner_dob);
    formDataObj.append('hotel_owner_address', formData.hotel_owner_address);
    ...
    try {
      ...
      alert('Error saving hotel owner: ' + error.message);
    } finally {
      setLoading(false);
    }
  };
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input
            ...
            value={formData.hotel_owner_name}
        ...
        <div>
          ...
          <input
            type="text"
            name="hotel_owner_nic"
            value={formData.hotel_owner_nic}
            onChange={handleInputChange}
            required
            ...
            placeholder="Enter hotel owner NIC"
          />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <input
            type="date"
            name="hotel_owner_dob"
            value={formData.hotel_owner_dob}
            onChange={handleInputChange}
            required
            className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          />
      ...
      <div>
        ...
        <textarea
          ...
          value={formData.hotel_owner_address}
          onChange={handleInputChange}
          required
          rows="1"
          className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          placeholder="Enter hotel owner address"
        />
      </div>

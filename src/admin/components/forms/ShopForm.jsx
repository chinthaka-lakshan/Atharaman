...
const ShopForm = ({ shop, onSave, onCancel, selectedOwner, submitting = false }) => {
  ...
  const restoreExistingImage = (imageId) => {
    ...
    if (shop) {
      const originalImage = shop.images.find(img => img.id === imageId);
      if (originalImage) {
  ...
  const handleSubmit = async (e) => {
    ...
    formDataObj.append('shop_name', formData.shop_name);
    formDataObj.append('nearest_city', formData.nearest_city);
    formDataObj.append('shop_address', formData.shop_address || '');
    ...
    newImages.forEach(img => {
      formDataObj.append('shopImage[]', img);
    });

    // Append images to remove (for edit mode)
    if (shop && imagesToRemove.length > 0) {
  ...
  const removedImages = shop ? shop.images.filter(img => imagesToRemove.includes(img.id)) : [];
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div>
        ...
        <textarea
          ...
          rows="5"
          className="w-full border border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          placeholder="Detailed description of the shop (optional)"
          maxLength="10000"
        />
        <p className="text-xs text-gray-500 mt-1">
      ...
      <div className="flex justify-end space-x-4 pt-6 border-t border-gray-200">
        ...
        >
          {submitting ? <Loader className="w-5 h-5 animate-spin" /> : (shop ? 'Update' : 'Create')}
        </button>
      </div>
    </form>
  );

...
const AllMyReview = () => {
  ...
  // Load reviews from the backend using the logged-in user's ID
  async function loadReview() {
    if (!user) {
    ...
    try {
      const response = await axios.get(`http://localhost:8080/review/find-by-user-id/${user.id}`);
      setReviews(response.data);
    } catch (error) {
      console.error('Error loading reviews:', error);
    }
